...
export const createArticle = async (articleData: Omit<ArticleModel, '_id' | 'createdAt' | 'updatedAt'>) => {
    const db = await getDb();
    const newArticle = {
        ...articleData,
...
export const updateArticle = async (id: string, articleData: Partial<ArticleModel>) => {
    const db = await getDb();
